  ...
  beforeEach(() => {
    ...
    const loginQueryMock = jest.fn(sql => {
      if (
        sql
          .toLowerCase()
          .includes(
            'select id, name, username, password, email from users where username'
          )
      ) {
        return Promise.resolve({
          rows: [
    ...
    const checkUserQueryMock = jest.fn((sql, params) => {
      if (sql.toLowerCase().includes('select 1 from users where username')) {
        return Promise.resolve({
          rows: params[0] === 'testuser' ? [{ exists: true }] : [],
        });
      }
      return Promise.resolve({ rows: [] });
    });
    ...
    mockPool.query.mockImplementation((sql, params) => {
      ...
      if (
        sqlLower.includes('select 1 from users where username = $1 limit 1')
      ) {
